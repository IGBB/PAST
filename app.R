...
sessions <- reactiveValues(names=list(), timing=list())
...
ui <- dashboardPage(
    ...
    dashboardBody(
        tags$script("
      Shiny.addCustomMessageHandler('inactivity', function(timer) {
        var t = setTimeout(logout, timer*1000);
        ...
        function logout() {
        Shiny.setInputValue('timeOut', timer)
        }
        
        function resetTimer() {
        clearTimeout(t);
        t = setTimeout(logout, timer*1000);  // time is in milliseconds (1000 is 1 second)
        }
        });
    "),
...
server <- function(input, output, session) {
    ...
    isolate(sessions$names <- append(sessions$names, session$token))
    observeEvent(sessions$names, {
        if (sessions$names[[1]] == session$token) {
            if (!(session$token %in% sessions$timing)) {
                session$sendCustomMessage("inactivity", 28800)
                isolate(sessions$timing <- append(sessions$timing, session$token))
            }
            try(removeNotification(session$token))
        } else if (sessions$names[[1]] != session$token) {
            queue_length = isolate(match(session$token, sessions$names))-2
            verb = ifelse(queue_length == 1, "is ", "are ")
            noun = ifelse(queue_length == 1, " session ", " sessions ")
            showNotification2(paste0("This instance is in use. Please try another session (listed ", 
                                    HTML("<a href='https://www.maizegdb.org/past'>here</a>"),
                                    " ) or wait until this instance is available.",
                                    HTML("<hr>"),
                                    ifelse(queue_length == 0, 
                                           "Your session is next.", 
                                           paste0("There ", verb, queue_length, noun, "ahead of you.")
                                    )
                            ), 
            duration = NULL,
            id=session$token)
    ...
    observeEvent(input$timeOut, { 
        showNotification(paste("Session timeout due to", input$timeOut/3600, "hours of inactivity."), 
                         duration = NULL,
    ...
    observeEvent(input$start_button, {
        ...
        withProgress(message = 'Beginning analysis', value = 0,detail="0%", {
            ...
            tryCatch({
                ...
                showModal(modalDialog(
                    title = "Error",
                    as.character(safeError(e)),
                    HTML("<br>"),
                    "\nPlease copy/paste this error and report it to ", 
                    HTML(paste0("<a href='mailto:thrash@igbb.msstate.edu?subject=PAST Error Report&body=", htmltools::htmlEscape(as.character(safeError(e))), ">thrash@igbb.msstate.edu</a>")), 
                    ".")
                )
            })
    ...
    session$onSessionEnded(function(){
        isolate(sessions$names[[match(session$token, sessions$names)]] <- NULL)
    })
}
