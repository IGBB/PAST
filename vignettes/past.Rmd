...
demo_effects_file = system.file("extdata", "effects.txt.xz", 
                                package = "PAST", mustWork = TRUE)
demo_effects_single_line = system.file("extdata", "effects.single_line.txt.xz", 
                                       package = "PAST", mustWork = TRUE)
demo_single_gwas_file = system.file("extdata", "single_gwas.txt.xz",
                                    package = "PAST", mustWork = TRUE)
...
```{r loading_gwas_data}
gwas_data_from_single_file <- load_GWAS_data(c(demo_single_gwas_file), input_type = "single")
gwas_data_from_two_files <- load_GWAS_data(c(demo_association_file, demo_effects_single_line), input_type = "two")
gwas_data_from_TASSEL <- load_GWAS_data(c(demo_association_file, demo_effects_file), input_type = "TASSEL")
# check equality
all.equal(gwas_data_from_single_file, gwas_data_from_two_files)
all.equal(gwas_data_from_single_file, gwas_data_from_TASSEL)
all.equal(gwas_data_from_two_files, gwas_data_from_TASSEL)
# assign gwas_data to gwas_data_from_TASSEL and use gwas_data in other examples
gwas_data = gwas_data_from_TASSEL
...
chromosome. Your LD data's Chromosome information should match your GFF 
...
```{r assigning_SNPs}
genes <-assign_SNPs_to_genes(gwas_data_from_single_file, 
                             LD, 
                             demo_genes_file,
                             c("gene"),
                             1000, 
                             0.8, 
                             2)
genes_data = as.data.frame(rtracklayer::readGFF(demo_genes_file, filter=list(type=c("gene"))))
genes_from_df <-assign_SNPs_to_genes(gwas_data_from_single_file, 
                                     LD, 
                                     genes_data,
                                     c("gene"),
                                     1000, 
