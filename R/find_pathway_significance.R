...
#'                                              "increasing",
#'                                              1000)
find_pathway_significance <- function(
    genes,
    pathways_file,
    ...
    effect <- . <- name <- effect0 <- rank0 <- pathway_id <- NULL
    permutation_mean <- value <- permutation_sd <- `p-value` <- NULL
    normalized_enrichment_score <- `q-value` <- NULL
    gene <- gene_id <- enrichment_score_observed <- enrichment_score <- NULL
        ...
        (paste0("enrichment_score", c(0:permutations))) :=
            lapply(
                ...
                function(column) {
                    ...
                    #   the previous gene from the rank of the current gene.
                    # The cumulative sum of the factors per row is calculated
                    #    and added to the previously calculated factor.
                    factor <- unlist(
                        .SD[, paste0("rank", column), with = FALSE] -
                            data.table::shift(
                                .SD[, paste0("rank", column),
                                    with = FALSE],
                                fill = 0) - 1)
                    factor <- factor + data.table::shift(
                        cumsum(factor),
                        fill = 0
                    )

                    # The factor is divided by the number of genes that are
                    #   not in the pathway to calculate the pmiss.
                    pmiss <- unlist(factor / (number_of_genes - .N))

                    # The effect of each gene in the pathway is divided by
                    #   the sum of the absolute value of every effect in the
                    #   pathway.
                    # A column is created to track the phit_base of the
                    #   previous gene in the pathway.
                    # The cumulative sum of the lagged phits per row is
                    #   calculated and added to the phit.
                    phit <- unlist(
                        abs(.SD[, paste0("effect", column), with = FALSE]) /
                            sum(abs(.SD[, paste0("effect", column),
                                        with = FALSE])))
                    phit <- phit + cumsum(data.table::shift(phit, fill = 0))

                    # The progress bar is updated.
                    # The pmiss is subtracted from the phit to calculate the
                    #   enrichment score per gene.
                    # The maximum enrichment score from the enrichment scores
                    #    of all genes in the pathway is returned.
    ...
    enrichment_scores[
        ...
        c(
            "gene_id",
            paste0("rank", c(0:permutations)),
            paste0("effect", c(0:permutations))
        ) := NULL
    ]
    ...
    enrichment_scores[
        ...
        normalized_enrichment_score :=
            (enrichment_score_observed - permutation_mean) /
            permutation_sd]
    enrichment_scores[
        ,
        `p-value` := 1 - stats::pnorm(normalized_enrichment_score)
    ]
    ...
    running_enrichment_scores[, enrichment_score := {
        ...
        # Calculate factors.
        # Factors are calculated by subtracting 1 + the rank of the previous
        #   gene from the rank of the current gene.
        # The cumulative sum of the factors per row is calculated and added to
        #   the previously calculated factor.
        factor <- unlist(
            .SD[, rank] - data.table::shift(.SD[, rank], fill = 0) - 1
        )
        factor <- factor + data.table::shift(cumsum(factor), fill = 0)

        # The factor is divided by the number of genes that are not in the
        #   pathway to calculate the pmiss.
        pmiss <- unlist(factor / (number_of_genes - .N))
        ...
        # The pmiss is subtracted from the phit to calculate the enrichment
        #   score per gene.
        phit - pmiss
    },
    by = pathway_id,
    ...
    running_enrichment_scores <- running_enrichment_scores[
        ,
        if (.N > gene_number_cutoff) .SD,
        by = pathway_id
    ]
