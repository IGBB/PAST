...
plot_enrichment_data <- function(enrichment_data,
  ...
  # Save the plot to the output directory, naming the image using the pathway ID.
  enrichment_data[get(significance_measure) <= significance_cutoff, {
    ...
    plot = ggplot2::ggplot(.SD,
      ...
      ggplot2::geom_line(stat = "identity", color = "#fe4365") +                       # Draw a line for running enrichment score.
      ggplot2::geom_rug(sides = "t", color = "#fe4365") +                              # Add marks for gene ranks.
      ggplot2::geom_vline(xintercept = intercept,                                      # Draw an intercept at the maximum running enrichment score.
                          color = "red",
                          linetype = "longdash") +
      ggplot2::ggtitle(title) +                                                        # Set the title.
      ggplot2::labs(x = "Gene Rank", y = "Running Enrichment Score") +                 # Set the x-axis and y-axis names.
      ggplot2::theme_minimal()                                                         # Set the theme to minimal.
    
    if (theme == "dark") {
      plot = plot + ggplot2::theme(                                                                  # Modify elements of the theme to create a dark theme.
        axis.text = ggplot2::element_text(color = "#a1a1a1"),
        plot.background = ggplot2::element_rect(color = "#a1a1a1", fill = "#222831"),
        panel.background = ggplot2::element_rect(color = "#a1a1a1", fill = "#222831"),
