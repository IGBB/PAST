...
assign_SNPs_to_genes <- function(gwas_data,
    ...
    genes = data.table::rbindlist(lapply(1:2, function(direction) {
        ...
        if (direction == 1) {
            ...
            initial_data[, c("temp_position", "temp_site") := .(position1, site1)]
            initial_data[, c("position1", "site1") := .(position2, site2)]
            initial_data[, c("position2", "site2") := .(temp_position, temp_site)]
            initial_data[, c("temp_position", "temp_site") := NULL]
        }
        ...
        data.table::setnames(linkage_with_effects, 
        ...
        data.table::setcolorder(linkage_with_effects,
                                c("locus", 
        ...
        data.table::setcolorder(single_SNPs,
                                c("locus", 
                                  "marker1", "position1", "site1", "effect1", "p-value1",
                                  "marker2", "position2", "site2", "effect2", "p-value2",
                                  "distance", "r_squared", "linked_snp_count"
        ...
        # If SNP1 and SNP2 are linked to each other and their effects have different 
        #   signs and they have equal p-values, choose the SNP with the largest
        #   absolute effect.
        tagSNPs[linked_snp_count == 1 & effect1 * effect2 < 0 & `p-value1` == `p-value2` & abs(effect1) > abs(effect2), SNP := "SNP1"]
        tagSNPs[linked_snp_count == 1 & effect1 * effect2 < 0 & `p-value1` == `p-value2` & abs(effect1) < abs(effect2), SNP := "SNP2"]
        ...
        if (debug) {
            tagSNPs = tagSNPs[, .(chromosome, position, effect, `p-value`, distance, linked_snp_count, represents)]  
        } else {
    ...
    genes = genes[, {
        ...
        if (debug) {
            ...
            subdata[, c("linked_snp_count", "represents") := list(negative + positive, SNPs)]
        } else {
