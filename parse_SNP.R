parse_SNP <- function(all_data, LD) {
  ...
  for (name in names(LD_upstream)){
    ...
    temp_data <- temp_data %>% mutate(Marker1 = paste0("S", Locus1, "_", Position1)) %>% 
      mutate(Marker2 = paste0("S", Locus1, "_", Position2))
    
    index <- c(0, cumsum(abs(diff(temp_data$Site2)) > 1))
    split(temp_data, index)
    
    block_SNPs <- chr_linked %>% group_by(Position1) %>% summarise(count = n()) %>% filter(count > 1)
